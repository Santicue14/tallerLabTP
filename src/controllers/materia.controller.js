...
const createMateria = (req,res) =>{
    ...
    return res.status(201).json({mensaje: "Materia creada con exito", materia: carrera.materias[carrera.materias.length-1]})
}
...
const getMateriasByCarrera = (req,res) =>{
    ...
    return res.status(200).json({carrera:carrera.nombre,materias: materiasById})
}
...
const deleteMateriaById = (req,res) =>{
    ...
    if(idx >= 0){
        const carrera = carreras.find(carrera => carrera.materias.some(materia => materia.id == id))
        const idxMateriaDentroDeCarrera = carrera.materias.findIndex(materia => materia.id == id)
